...
class StageLongCall(Stage):
    def create(self):
        ...
        stage1.e_amount = self.buy_calls[0].net_price * self.buy_calls[0].qty * -100
...
class StageNakedCall(Stage):
    def create(self):
        ...
        stage1.e_amount = self.sell_calls[0].net_price * self.sell_calls[0].qty * -100
...
class StageLongPut(Stage):
    def create(self):
        ...
        stage2.e_amount = self.buy_puts[0].net_price * self.buy_puts[0].qty * -100
        stage2.gt_stage = 'MAX_LOSS'
        stage2.gt_amount = self.buy_puts[0].net_price * self.buy_puts[0].qty * -100
...
class StageNakedPut(Stage):
    def create(self):
        ...
        stage2.e_amount = self.sell_puts[0].net_price * self.sell_puts[0].qty * -100
        stage2.gt_stage = 'MAX_PROFIT'
        stage2.gt_amount = self.sell_puts[0].net_price * self.sell_puts[0].qty * -100

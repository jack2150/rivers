...
class StageLongCallVertical(Stage):
    def create(self):
        ...
        s1.e_stage = 'MAX_LOSS'
        s1.e_amount = s1.lt_amount
        s1.gt_stage = 'LOSS'
        s1.gt_amount = s1.lt_amount
        ...
        s3.lt_stage = 'PROFIT'
        s3.lt_amount = ((self.sell_calls[0].strike - self.buy_calls[0].strike
                         - self.buy_calls[0].net_price) * self.buy_calls[0].qty * 100)
        s3.e_stage = 'MAX_PROFIT'
        s3.e_amount = s3.lt_amount
        s3.gt_stage = 'MAX_PROFIT'
        s3.gt_amount = s3.lt_amount
...
class StageShortCallVertical(Stage):
    def create(self):
        ...
        s3.lt_stage = 'LOSS'
        s3.lt_amount = ((self.buy_calls[0].strike - self.sell_calls[0].strike
...
class StageLongPutVertical(Stage):
    def create(self):
        ...
        s3.lt_stage = 'LOSS'
        s3.lt_amount = self.sell_puts[0].net_price * self.sell_puts[0].qty * 100
        s3.e_stage = 'MAX_LOSS'
        s3.e_amount = s3.lt_amount
        s3.gt_stage = 'MAX_LOSS'
        s3.gt_amount = s3.lt_amount

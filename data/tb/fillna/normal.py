...
class FillNaNormal(object):
    ...
    def count_missing(self):
        """
        Count every option_code missing in df_normal using df_stock dates
        """
        print '-' * 70
    ...
    def fill_missing(self):
        ...
        print '-' * 70
        df_data = self.df_normal[self.df_normal['option_code'].isin(self.df_missing['option_code'])]
    ...
    def remove_missing(self):
        ...
        df_normal = self.df_normal.copy()
        """:type: pd.DataFrame"""
        length0 = len(df_normal)
        print output % ('STAT', 'Length df_normal before remove: %d' % length0)
        df_normal = df_normal[~df_normal['option_code'].isin(remove_codes)]
        print output % ('STAT', 'Length df_normal after remove: %d' % len(df_normal))
        print output % ('STAT', 'Total remove: %d' % (length0 - len(df_normal)))
    ...
    def save(self):
        ...
        print '-' * 70
        print output % ('SAVE', 'Save df_normal with fillna data')
        print '-' * 70

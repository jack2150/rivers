...
def create_order(df_stock, df_signal, moneyness=('OTM', 'ITM', 'ATM'),
    ...
    for date0a, date0b, (index, signal) in zip(dates0a, dates0b, df_signal.iterrows()):
        ...
        if date0a == date0b and date0a and date0b:
            ...
            close0 = option0b.ask - option0a.bid  # reverse 0 1 ask bid
            ...
            if close0:
                ...
                if option1a and option1b:
                    if int(expire):
                        ...
                        ask1 = np.float(price1 - np.float(option0a.contract.strike))
                        ask1 = ask1 if ask1 > 0 else 0.0
                        bid1 = np.float(price1 - np.float(option0b.contract.strike))
                        bid1 = bid1 if bid1 > 0 else 0.0
                        close1 = ask1 - bid1
                    else:
                        # when close, reverse ask bid into bid ask
                        close1 = np.float(option1b.bid - option1a.ask)
            ...
            if option0a and option0b and option1a and option1b and close0:
                data.append({
                    ...
                    'option0a': np.float(option0b.bid),
                    'option0b': np.float(option0a.ask),
                    'option1a': np.float(option1b.ask),
                    'option1b': np.float(option1a.bid),
                    'close0': np.float(close0),  # buy using ask
                    'close1': np.float(close1),  # sell using bid
                    'option_code0': option0b.contract.option_code,
                    'option_code1': option0a.contract.option_code,
                    'strike0': np.float(option0b.contract.strike),
                    'strike1': np.float(option0a.contract.strike),
                    'dte0': np.int(option1a.dte),
                    'dte1': np.int(option0a.dte)
                })

...
def create_order(df_stock, df_signal, order=('gtc', 'close'), profit_pct=0, loss_pct=0):
    """
    ...
    for index, signal in df_signal.iterrows():
        ...
        if signal['signal0'] == 'BUY':
            ...
            #print signal['close0'], limit, stop_loss

            if order == 'close':
                df_temp['sold1'] = df_temp['close'] >= limit
                df_temp['sold2'] = df_temp['close'] <= stop_loss
                ...
                try:
                    ...
                    if df_temp.ix[exit_date]['sold1']:
                        limit_hit = True
                    elif df_temp.ix[exit_date]['sold2']:
                        stop_loss_hit = True
                except IndexError:
                ...
                df_temp['sold2'] = df_temp.apply(
                    lambda x: any([
                        True if x[n] <= stop_loss else False
                        for n in ('open', 'high', 'low', 'close')
                    ]),
                    axis=1
                )

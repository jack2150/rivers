...
def handle_data(df, move=('up', 'down'), pct_from=0, pct_to=0, period=0, count=0, bdays=0):
    df0 = df.copy()
    ...
    df0['hit'] = df0['pct_chg'].apply(f)
    df0['roll_hit'] = pd.rolling_apply(df0['hit'], period, lambda x: sum(x))
    df0['found'] = (df0['hit']) & (df0['roll_hit'] == count)

...
def create_order(df_signal, df_stock, df_all,
    ...
    for index, data in df_signal0.iterrows():
        try:
            ...
            max_loss = data['close0'] * (stop / 100.0)
            if data['signal0'] == 'BUY':
            ...
            # buy sell are same
            if len(df_stop):
                exit_date = pd.to_datetime(df_stop['date'].iloc[0])
                ...
                if data['signal0'] == 'BUY':
                    ...
                    data['price0b'] = -df_stop['sell0'].iloc[0]
                    data['price1b'] = df_stop['buy1'].iloc[0]
                else:  # SELL
                    data['price0a'] = -option0a['sell']
                    data['price1a'] = option1a['buy']
                    data['price0b'] = df_stop['buy0'].iloc[0]
                    data['price1b'] = -df_stop['sell1'].iloc[0]
            else:
                data['stock0'] = data['close0']
                data['stock1'] = data['close1']
                if data['signal0'] == 'BUY':

...
class TestComboCS(TestStrategy2):
    def setUp(self):
        TestStrategy2.setUp(self)

        self.symbol = 'VIX'
        self.ready_signal0()
        self.trade = Trade.objects.get(name='Combo -Pct')
    ...
    def get_trade(self):
        ...
        try:
            ...
            df_trade = self.backtest.create_order(
                ...
                **{'side': 'buy', 'cycle': 0, 'pct0': 0, 'pct1': 0}
            )
    ...
    def test_make_order(self):
        ...
        for side in ('follow', 'reverse', 'buy', 'sell')[2:]:
            kwargs = {'side': side, 'cycle': 0, 'pct0': 0, 'pct1': 0}
            print kwargs
            df_trade = self.backtest.create_order(
                self.df_signal,
                self.backtest.df_all,
    ...
    def test_join_data(self):
        ...
        df_list = self.backtest.join_data(
            df_trade[-5:], self.backtest.df_stock, self.backtest.df_all
        )

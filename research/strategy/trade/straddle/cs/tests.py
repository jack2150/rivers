...
class TestStrangleCS(TestStrategy2):
    def setUp(self):
        ...
        self.symbol = 'AIG'
        self.ready_signal0()
        self.trade = Trade.objects.get(name='Straddle -CS')
    ...
    def test_make_order(self):
        ...
        report = []
        for side in ('follow', 'reverse', 'buy', 'sell')[2:3]:
            for i in [0]: # range(-3, 2):
                kwargs = {'side': side, 'cycle': 0, 'strike': i}
                print kwargs
                df_trade = self.backtest.create_order(
                    self.df_signal,
                    self.backtest.df_all,
    ...
    def test_join_data(self):
        ...
        df_list = self.backtest.join_data(
            df_trade[:3], self.backtest.df_stock, self.backtest.df_all, self.backtest.df_iv
        )

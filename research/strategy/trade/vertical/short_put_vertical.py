...
def create_order(df_stock, df_signal, moneyness=('OTM', 'ITM', 'ATM'),
    ...
    for date0a, date0b, (index, signal) in zip(dates0a, dates0b, df_signal.iterrows()):
        ...
        if date0a == date0b and date0a and date0b:
            ...
            close0 = option0b.ask - option0a.bid
            ...
            if close0:
                ...
                if option1a and option1b:
                    if int(expire):
                        ...
                        ask1 = np.float(np.float(option0a.contract.strike) - price1)
                        ask1 = ask1 if ask1 > 0 else 0.0
                        bid1 = np.float(np.float(option0b.contract.strike) - price1)
            ...
            if option0a and option0b and option1a and option1b and close0:
                data.append({
                    ...
                    'option0a': np.float(option0b.ask),
                    'option0b': np.float(option0a.bid),
                    'option1a': np.float(option1b.bid),
                    'option1b': np.float(option1a.ask),

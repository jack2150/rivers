...
def create_order(df_signal, df_all,
    ...
    for index, data in df_signal0.iterrows():
        try:
            ...
            df_code = df_all.query('option_code == %r & date >= %r & date <= %r' % (
                option0['option_code'], data['date0'], data['date1']
            ))
            ...
            if data['signal0'] == 'BUY':
                ...
                limit_bid = data['close0'] * (1 + (limit_pct / 100.0))
                stop_bid = data['close0'] * (1 - (stop_pct / 100.0))
                data['limit'] = limit_bid
                data['stop_loss'] = stop_bid

                df_exit = df_code[
                    (df_code['bid'] <= stop_bid) | (df_code['bid'] >= limit_bid)
                ]
                if len(df_exit):
                    data['date1'] = df_exit['date'].iloc[0]
                    data['close1'] = -df_exit['bid'].iloc[0]
            else:  # SELL
                data['bp_effect'] = (data['close0'] / 5.0) * 100
                data['close0'] = -option0['bid']
                data['close1'] = option1['ask']
                limit_ask = -data['close0'] * (1 - (limit_pct / 100.0))
                stop_ask = -data['close0'] * (1 + (stop_pct / 100.0))
                data['limit'] = limit_ask
                data['stop_loss'] = stop_ask

                df_exit = df_code[
                    (df_code['ask'] >= stop_ask) | (df_code['ask'] <= limit_ask)
                ]
                if len(df_exit):
                    data['date1'] = df_exit['date'].iloc[0]
                    data['close1'] = df_exit['ask'].iloc[0]

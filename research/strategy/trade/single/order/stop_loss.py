    ...
    for index, data in df_signal0.iterrows():
        try:
            ...
            if data['signal0'] == 'BUY':
                ...
                exit_bid = data['close0'] * (1 - (stop_pct / 100.0))
                data['stop_loss'] = exit_bid

                df_exit = df_code[df_code['bid'] <= exit_bid]
                if len(df_exit):
                ...
                exit_ask = -data['close0'] * (1 + (stop_pct / 100.0))
                data['stop_loss'] = exit_ask

                df_exit = df_code[df_code['ask'] >= exit_ask]
                if len(df_exit):
                    data['date1'] = df_exit['date'].iloc[0]
